...
def load_modelLstm(fileName):

    config = Config()

    
    dataGen = DataGenerator(fileName, config)
    dataGen.gen_attr(True)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for params in dataGen.next_batch(test_seqs):
            ...
            saver = tf.compat.v1.train.import_meta_graph("home/modelDktLstm/my-model-800.meta")
            saver.restore(sess, tf.train.latest_checkpoint("home/modelDktLstm/"))
            ...
            pred, binary_pred = sess.run([pred, binary_pred],
